    ...
    <div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 mb-6">
        ...
        <div className="w-full bg-gray-200 dark:bg-slate-700 rounded-full h-2">
          <div
            className="bg-tobie-600 dark:bg-tobie-500 h-2 rounded-full transition-all duration-500"
            style={{
      ...
      {loading ? (
        <div className="p-12 text-center text-gray-400 dark:text-gray-500">Loading review queue...</div>
      ) : !current ? (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-12 text-center">
          <CheckCircle className="w-16 h-16 text-green-300 dark:text-green-600 mx-auto mb-4" />
          <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">All caught up!</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {current.transcripts.map((t, i) => (
                <div key={i} className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-sm text-gray-800 dark:text-gray-200 flex-1">
                      {t.user_message}
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-7 h-7 bg-gray-100 dark:bg-slate-700 rounded-full flex items-center justify-center flex-shrink-0">
                      <Bot className="w-3.5 h-3.5 text-gray-600 dark:text-gray-400" />
                    </div>
                    <div className="flex-1">
                      <div className="bg-gray-50 dark:bg-slate-900 rounded-lg p-3 text-sm text-gray-800 dark:text-gray-200 whitespace-pre-wrap">
                        {t.bot_response}
                      </div>
                      {t.sources_used?.length > 0 && (
                        <div className="mt-1 flex flex-wrap gap-1">
                          {t.sources_used.map((s) => (
                            <span key={s} className="text-xs bg-tobie-50 dark:bg-tobie-900/30 text-tobie-700 dark:text-tobie-300 px-2 py-0.5 rounded inline-flex items-center gap-1">
          ...
          {/* Verdict Panel */}
          <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 h-fit">
            <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-4">Verdict</h3>
            <div className="space-y-3">
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-3 py-2 border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-gray-100 rounded-lg text-sm placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-tobie-500 resize-none"
                />
              </div>
              ...
              <button
                ...
                className="w-full px-4 py-2 text-gray-600 dark:text-gray-400 text-sm font-medium border border-gray-200 dark:border-slate-600 rounded-lg hover:bg-gray-50 dark:hover:bg-slate-700 flex items-center justify-center gap-2"
              >
